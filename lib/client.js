...
((scope) => {
    /**
     * @type {String}
     * Auto-detect the ServiceWorker script url.
     * Replace the current script entry with the sw path.
     */
    const SW_URL = scope.UNCHAINED_SW_URL || (() => {
        // detect the current script source url.
        let src = document.currentScript.src;
        // replace the last entry with the sw path.
    ...
    /**
     * @type {String}
     * Auto-detect ServiceWorker registration options.
     * Use the <base> href as SW scope.
     */
    ...
    class UnchainedClient {
         ...
         * @return {Promise}
         */
        static register(path, options, config) {
            if (this.checkSupport()) {
                // SW is supported.
                if (typeof path !== 'string') {
                    // Use first parameter as Unchained configuration.
                    config = path;
                    // Auto-detect path and options.
                    path = SW_URL;
                    options = SW_OPTIONS;
                }
                if (config) {
