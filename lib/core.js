...
((scope) => {
    ...
    class Unchained {
        ...
        static transform(file, input, options = {}) {
            ...
            options.filename = options.hasOwnProperty('filename') ? options.filename : file.url;
            ...
            // https://github.com/babel/babel/issues/4824
            // options.comments = false;
            // options.minified = false;
            // if (input.ast) {
            //     // use ast transformation.
            //     return Babel.transformFromAst(input.ast, input.code || file.content, options)
            // }
        ...
        async transform() {
            ...
            for (let plugin of plugins) {
                ...
                result = await plugin.transform(this.file, result);
            }
        ...
        async resolve(result) {
            if (this.checkDependencies(result)) {
                ...
                result = Unchained.transform(this.file, result, {
                    plugins: [
                        {
                            visitor: {
        ...
        async finalize(result) {
            // filter plugin with `finalize` method.
            let plugins = this.filterPlugins('finalize');
            for (let plugin of plugins) {
                // exec plugin finalize method.
                result = await plugin.finalize(this.file, res);
            }
            // inline sourcemaps
            return Promise.resolve(
                Unchained.transform(this.file, result, {
                    sourceMaps: 'inline',
                }).code
            );
        }
