...
((scope) => {
    ...
    class Unchained {
        ...
        /**
         * A wrap around `Babel.transform` method, but only for traversing.
         *
         ...
         * @return {void}
         */
        static traverse(file, input, options = {}) {
            if (input.ast) {
                // use AST transformation.
                Babel.transformFromAst(input.ast, input.code, options);
            } else {
                // use code transformation.
                Babel.transform(input.code, options);
            }
        }
        ...
        async resolve(result) {
            if (this.checkDependencies(result)) {
                ...
                result = Unchained.traverse(this.file.url, result, {
                    plugins: [
                        {
                            visitor: {
